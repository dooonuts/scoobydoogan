...
def make_edge_smooth(dataset_name, img_size) :
    ...
    smooth_file_list = glob('dataset/{}/{}/*.*'.format(dataset_name, 'smooth'))
    file_list_truncated = [curr_file.split("style")[1] for curr_file in file_list]
    smooth_file_list_truncated = [curr_file.split("smooth")[1] for curr_file in smooth_file_list]
    file_list_diff = list(set(file_list_truncated) - set(smooth_file_list_truncated))
    file_list = ["dataset/"+dataset_name+"/style"+curr_file for curr_file in file_list_diff]
