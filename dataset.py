    ...
    def __init__(self, args, transform=None):
        ...
        for opt in [self.photo, self.style, self.smooth]:
            if(opt == self.photo):
                ...
                folder = opt
                files = os.listdir(folder)
            self.image_files[opt] = [os.path.join(folder, fi) for fi in files if fi.endswith(".jpg")]
            # self.image_files[opt] = [f for f in os.listdir(file_dir) if (f.endswith(".jpg") or f.endswith(".tiff"))]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default="Hayao/")
    parser.add_argument('--debug_samples', default=0)
    args = parser.parse_args()
    # args.data_dir = "../AnimeGAN/dataset/Hayao/smooth"
    # anime_loader = DataLoader(AnimeDataSet('../AnimeGAN/dataset/Hayao/smooth'), batch_size=2, shuffle=True)
    anime_loader = DataLoader(AnimeDataSet(args), batch_size=2, shuffle=True)
    
    # print(iter(anime_loader).next().size())
