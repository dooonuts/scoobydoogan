...
from utils.common import load_optimizer
...
def parse_args():
    ...
    parser.add_argument('--lr-g', type=float, default=2e-5) # likely too high, will need to decrease
    parser.add_argument('--lr-d', type=float, default=4e-5) # likely too high, will need to decrease
    ...
    parser.add_argument('--load-optimizer', type=bool, default=False, help='Load Optimizer From Checkpoint')
...
def main(args):
    ...
    if(args.load_optimizer):
        try: 
            load_optimizer(optimizer_g, G, args.checkpoint_dir)
            print("G optimizer loaded")
            load_optimizer(optimizer_d, D, args.checkpoint_dir)
            print("D optimizer loaded")
    ...
    for e in range(start_e, args.epochs):
        ...
        for img, anime, anime_gray, anime_smt_gray in bar:
            ...
            writer.add_scalar("Generator Loss", avg_adv, e)
            writer.add_scalar("Grayscale Loss", avg_gram, e)
            writer.add_scalar("Color Loss", avg_color, e)
            writer.add_scalar("Content Loss", avg_content, e)
            writer.add_scalar("Discriminator Loss", avg_adv_d, e)
            writer.flush()
